...
class Node:
    def __init__(self, game_map, parent_node=None,cost=0,heuristic=0,g=0):
...
class MainGame:
    ...
    def create_game(self):   
        ...
        if  who_play=="hc":
          self.solve_hill_climbing(game_map) 
        if  who_play=="a":
          self.solve_A_star(game_map)   
    ...
    def solve_ucs(self, map):
     ...
     current_node = Node(map, None)
     current_node.g = 0
     current_node.cost=current_node.g
     pqueue.put((current_node.g, current_node))  
     stone_type = input("stone type:")

     while not pqueue.empty():  
         g, current_node = pqueue.get()  
         if current_node.game_map not in visited:
             if current_node.game_map.check_game():
                 path = self.get_parent_of_node(current_node)
                 for step in path:
             ...
             for child_node in self.stone_move_auto(current_node, stone_type):
                 if child_node.game_map not in visited:
                     child_node.g = g + 1
                     child_node.cost=child_node.g
                     pqueue.put((child_node.g, child_node))  
    ...
    def solve_hill_climbing(self, map):
     queue = deque()  
     visited = set()
     current_node = Node(map, None)
     queue.append(( self.heuristic(current_node), current_node))
     ...
     while  queue:
         ...
         if current_node.game_map not in visited:
             ...
             for child_node in childs:
                 if child_node.game_map not in visited:
                     cost = self.heuristic(child_node)
                     if cost<h:
                         queue.append((cost, child_node))

     print("Solution not found")

    def solve_A_star(self,map):
     pqueue = PriorityQueue()   
     visited = set()  
     childs = []
     current_node = Node(map, None)
     current_node.heuristic=self.heuristic(current_node)
     current_node.g=0
     current_node.cost=current_node.g+current_node.heuristic
     pqueue.put((current_node.cost,current_node.g, current_node))  
     stone_type = input("stone type:")

     while not pqueue.empty():  
         f, g,current_node = pqueue.get()  
         ...
         if current_node.game_map not in visited:
             ...
             for child_node in childs:
                 if child_node.game_map not in visited:
                     child_node.heuristic=self.heuristic(child_node)
                     child_node.g=g + 1 
                     child_node.cost = child_node.g+child_node.heuristic
                     pqueue.put((child_node.cost,child_node.g, child_node))  
    ...
    def heuristic(self, node):
      h=0  
      for home in node.game_map.homes:
            if 0 <= home[0] < node.game_map.size and 0 <= home[1] < node.game_map.size:  
                if node.game_map.new_map [home[0]][home[1]]  not in {"negative", "positive", "metal"}: 
                      h+=1
      return h                
    ...
    #  max_distance = 0
    #  magnet = None
    #  if node.game_map.positive:
    #      magnet = node.game_map.positive
    #  elif node.game_map.negative:
    #      magnet = node.game_map.negative

    #  if not magnet:
    #      return 0  

    #  for home in node.game_map.homes:
    #      distance = abs(home[0] - magnet.x) + abs(home[1] - magnet.y)
